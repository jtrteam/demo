...
OPC.prototype = {
    initialize: function (form, urls, agreement) {
        ...
        if (typeof shippingMethod === "object") this.sectionsToValidate.push(shippingMethod);
    ...
    _addEventListeners: function () {
        $("login-form") && $("login-form").observe("submit", function (e) {
            ...
            new Ajax.Request($("login-form").action, {
                ...
                onSuccess: function (transport) {
                    ...
                    if (response.error) OPC.Messenger.add(response.error, "login-form", "error");
        ...
        $("forgot-password-form") && $("forgot-password-form").observe("submit", function (e) {
            Event.stop(e);
            if (!forgotForm.validator.validate()) return;
            ...
            new Ajax.Request($("forgot-password-form").action, {
                ...
                onSuccess: function (transport) {
                    ...
                    var response = transport.responseText.evalJSON();
                    if (response.error) OPC.Messenger.add(response.error,
    ...
    save: function () {
        ...
        var isValid = true;
        if (!this.validator.validate()) isValid = false;
        for (i in this.sectionsToValidate) {
            if (typeof this.sectionsToValidate[i] === "function") continue;
            if (!this.sectionsToValidate[i].validate()) isValid = false
        }
        ...
        if (!isValid) {
            ...
            validationMessages.each(function (el) {
                ...
                if (offset.top < 0 || (offset.top > viewportSize.height || (offset.left < 0 || offset.left > viewportSize.width))) hiddenMessages.push(el);
                else needToScroll = false
            });
            if (needToScroll) Effect.ScrollTo(validationMessages[0], {
                duration: 1,
                offset: -20
            });
            return
        ...
        if (response.update_section)
            for (var i in response.update_section) {
                ...
                if (ch_obj != null) {
                    ...
                    if (i === "shipping-method") shippingMethod.addObservers()
                }
            }
        if (response.duplicateBillingInfo) shipping.syncWithBilling();
        if (response.reload_totals) checkout.update({
            "review": 1
        });
        return false
    },
    ...
    blockopc: function () {
        var request = new Ajax.Request(this.updateUrl, {
            method: "post",
            onSuccess: this.blockResponse.bind(this),
            onFailure: this.blockResponse.bind(this),
            parameters: {
                blockopc: "blockopc"
            }
        })
    ...
    blockResponse: function (response) {
        ...
        if (response.redirect) {
            ...
            location.href = checkout.successUrl + "na/yes";
            return true
        }
        return false
    },
...
BillingAddress.prototype = {
    initialize: function () {
        $("billing:country_id") && $("billing:country_id").observe("change", function () {
            ...
            if ($("shipping:same_as_billing") && $("shipping:same_as_billing").checked) shipping.syncWithBilling();
            checkout.update({
                "payment-method": 1,
        ...
        $("billing:region_id") && $("billing:region_id").observe("change", function () {
            ...
            } else if (!$("shipping:same_as_billing")) checkout.update({
                "review": 1
            })
        });
    ...
    setCreateAccount: function (flag) {
        if (flag) $("register-customer-password").show();
        ...
        if (!$("billing_customer_address") || !$("billing_customer_address").value) {
            ...
            for (var elemIndex in arrElements)
                if (arrElements[elemIndex].id) {
                    var sourceField = $(arrElements[elemIndex].id.replace(/^shipping:/,
                        "billing:"));
                    if (sourceField) arrElements[elemIndex].value = sourceField.value
                }
            shippingRegionUpdater.update();
            $("shipping:region_id").value = $("billing:region_id").value;
            $("shipping:region").value = $("billing:region").value
        } else $("shipping_customer_address").value = $("billing_customer_address").value
    },
...
ShippingMethod.prototype = {
    ...
    validate: function () {
        ...
        if (methods.length == 0) {
            OPC.Messenger.add(Translator.translate("Your order cannot be completed at this time as there is no shipping methods available for it. Please make neccessary changes in your shipping address."),
                "checkout-shipping-method-load", "error");
            return false
        }
...
Payment.prototype = {
    ...
    switchMethod: function (method) {
        if (this.currentMethod && $("payment_form_" + this.currentMethod)) {
            ...
            for (var i = 0; i <
                elements.length; i++) elements[i].disabled = true;
    ...
    beforeValidate: function () {
        ...
        this.beforeValidateFunc.each(function (validate) {
            hasValidation = true;
            if (validate.value() == false) validateResult = false
        }.bind(this));
        if (!hasValidation) validateResult =
            false;
    ...
    afterValidate: function () {
        ...
        if (!hasValidation) validateResult = false;
        return validateResult
    },
...
OPC.Window.prototype = {
    ...
    updateSize: function () {
        ...
        if ("auto" === this.config.width && size.width > this.config.maxWidth) this.content.setStyle({
            width: this.config.maxWidth + "px"
        });
        if ("auto" === this.config.height && size.height > this.config.maxHeight) this.content.setStyle({
    ...
    _attachEventListeners: function () {
        ...
        if (this.config.triggers)
            if (this.config.triggers.length != 0)
                for (var i in this.config.triggers) this.config.triggers[i].el.each(function (el) {
    ...
    _onKeyPress: function (e) {
        var code = e.keyCode;
        if (code == Event.KEY_ESC) this.hide()
    }
};
