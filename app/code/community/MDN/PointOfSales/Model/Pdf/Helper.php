...
abstract class MDN_PointOfSales_Model_Pdf_Helper extends Mage_Sales_Model_Order_Pdf_Abstract {
    ...
    protected function insertLogo(&$page, $StoreId = null) {
        $image = Mage::getStoreConfig('pointofsales/pdf/logo', $StoreId);
        if ($image) {
            $image = Mage::getStoreConfig('system/filesystem/media') . '/upload/image/' . $image;
            if (is_file($image)) {
    ...
    /**
     * Retourne la hauteur d'un texte en multiligne
     *
     * @param unknown_type $page
    ...
    protected function DrawMultilineText(&$page, $Text, $x, $y, $Size, $GrayScale, $LineHeight, $allowNewPage = true) {
        ...
        foreach (explode("\n", $Text) as $value) {
            if ($value !== '') {
                ...
                if (($y < $this->_BLOC_FOOTER_HAUTEUR) && $allowNewPage) {

                    $savedFont = $page->getFont();
                    $savedFontSize = $page->getFontSize();

                    $this->drawFooter($page, mage::helper('PointOfSales/User')->getStoreId());
                    $page = $this->NewPage();
    ...
    public function drawFooter(&$page, $StoreId = null) {
        ...
        $this->DrawMultilineText($page, Mage::getStoreConfig('pointofsales/pdf/pdf_footer', $StoreId), 20, $this->_BLOC_FOOTER_HAUTEUR, 10, 0, 15, false);
    }
    ...
    public function AddAddressesBlock(&$page, $LeftAddress, $RightAddress, $TxtDate, $TxtInfo) {
        ...
        $this->DrawMultilineText($page, $RightAddress, $this->_PAGE_WIDTH / 2 - 40, $this->y, 14, 0.4, 16);
