...
class Madia_Adyen_Model_Process extends Mage_Core_Model_Abstract {
    ...
    const LOCKING_MAX_CONCURRENT = 2;
    private $lockProc;
    ...
    public function processResponse($soapItem = null) {
        ...
        Filelock::$lockpath = Mage::getConfig()->getVarDir('locks') . DS;
        $lock = new Filelock( "AdyenNotificationLock" );
		...
		//$lock_id = self::LOCKING_PROCESS_ID.(($varienObj->getData('merchantReference'))%self::LOCKING_MAX_CONCURRENT);
		//$this->lockAndBlock($lock_id);
...
//        $this->lockProc->unlock();
	...
	public $_lockFile = null;
	public $_isLocked = null;
   /**
    ...
    protected function _getLockFile($id)
    {
    	if ($this->_lockFile === null) {
    		...
    		$file = $varDir . DS . 'index_process_'.$id.'.lock';
    		
    		// throw excception if directory has no write access
    		if (!is_writable($varDir)) {
    			// log file is not writable log error
    			$errorMsg = Mage::helper('adyen')->__('The file:' . $file . ' is not writable');
...
//    		}
     ...
     */
    public function lockAndBlock($id)
    {
    	$this->_isLocked = true;
    	flock($this->_getLockFile($id), LOCK_EX);
    	return $this;
    }
